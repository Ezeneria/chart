...
const prisonChart = chartFactory({
  ...
  prisonChart.updateAxisElements = function() {
      this.xAxisElement = this.container.select('.axis.x')
        .attr('transform', `translate(0, ${this.innerHeight()})`)
        .call(this.xAxis);
  
    this.yAxisElement = this.container.select('.axis.y')
        // .classed('axis y', true)
        .call(this.yAxis);
  }
  
  prisonChart.update = function(_data) {
